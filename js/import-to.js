...
(function(){
  ...
  function parseRows(data){const subjects=subjectColumns(data),result=[],errors=[];data.forEach((o,i)=>{const email=value(o,['email','e-mail','email siswa']),name=value(o,['nama','name','student','student name']),s=matchStudent(email,name);if(!email&&!name){errors.push({row:i+2,msg:'Email/nama kosong'});return;}if(!s){errors.push({row:i+2,msg:'Siswa tidak ditemukan berdasarkan email/nama'});return;}const directScore=value(o,['nilai','score']),subtest=value(o,['subtest','subtes','mapel','subject']);if(directScore&&subtest){const n=Number(String(directScore).replace(',','.'));if(Number.isFinite(n))result.push({student_id:masterId(s),student_name:masterName(s),email:masterEmail(s),subtest_name:subtest,score:n});else errors.push({row:i+2,msg:'Nilai tidak valid'});return;}let count=0;subjects.forEach(k=>{const raw=o[k];if(raw!==''&&raw!==undefined){const n=Number(String(raw).replace(',','.'));if(Number.isFinite(n)){result.push({student_id:masterId(s),student_name:masterName(s),email:masterEmail(s),subtest_name:k,score:n});count++;}}});if(!count)errors.push({row:i+2,msg:'Tidak ada nilai yang terbaca'});});return {result,errors};}
  function render(){const stat=document.getElementById('importStats'),status=document.getElementById('importStatus'),head=document.getElementById('importHead'),body=document.getElementById('importBody');if(!stat)return;const unique=new Set(parsed.map(x=>x.student_id));stat.innerHTML=[['Baris',rows.length],['Siswa cocok',unique.size],['Nilai',parsed.length],['Masalah',Math.max(0,rows.length-unique.size)]].map(x=>`<div class="import-stat"><span>${x[0]}</span><b>${x[1]}</b></div>`).join('');status.className='import-status '+(parsed.length?'ok':'warn');status.textContent=parsed.length?`Siap: ${unique.size} siswa, ${parsed.length} nilai. Nama mengikuti master berdasarkan email.`:'Belum ada data valid.';head.innerHTML='<tr><th>Siswa</th><th>Email</th><th>Subtes</th><th>Nilai</th><th>Status</th></tr>';body.innerHTML=parsed.slice(0,100).map(x=>`<tr><td>${esc(x.student_name)}</td><td>${esc(x.email)}</td><td>${esc(x.subtest_name)}</td><td>${esc(x.score)}</td><td><span class="tag tag-ok">COCOK</span></td></tr>`).join('');}
  function process(data){rows=data;const p=parseRows(data);parsed=p.result;render();if(p.errors.length){const st=document.getElementById('importStatus');st.className='import-status warn';st.textContent=`${parsed.length} nilai valid. ${p.errors.length} baris perlu dicek; data bermasalah tidak dikirim.`;}}
  ...
  function bind(){
    ...
    document.getElementById('importClearBtn')?.addEventListener('click',()=>{rows=[];parsed=[];const ta=document.getElementById('importPaste');if(ta)ta.value='';render();});
