...
(function(){
  ...
  function ensureMenu(){
    ...
    b.setAttribute('aria-label','Import TO');b.style.display='block';b.style.visibility='visible';b.style.opacity='1';
  ...
  function masterEmails(s){const values=[s?.email,s?.email_address,s?.email_siswa,s?.email_student,s?.mail,s?.username,s?.Email,s?.['Email Siswa'],s?.['Email Student'],s?.['email siswa'],s?.['email student']];return values.concat(Object.values(s||{})).map(emailNorm).filter(v=>v.includes('@'));}
  function masterEmail(s){return masterEmails(s)[0]||'';}
